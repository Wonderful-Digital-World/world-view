...
// Exercise the unified registry under the codex adapter: TINYPLACE_HARNESS pins
// the harness so harnessDataDir() reads TINYPLACE_CODEX_HOME and labels are codex:n.
process.env.TINYPLACE_HARNESS = "codex";
process.env.TINYPLACE_CODEX_HOME = mkdtempSync(join(tmpdir(), "tinyplace-reg-"));
...
writeRaw(A, "codex:1", { pid: process.pid, updatedAt: now() });
writeRaw(A, "codex:2", { pid: process.pid, updatedAt: now() });
writeRaw(A, "codex:9", { pid: deadPid, updatedAt: now() }); // dead → not live
writeRaw(A, "codex:8", { pid: process.pid, updatedAt: ago(60_000) }); // stale → not live
...
expect("liveSessions sorted by label", live[0].label === "codex:1" && live[1].label === "codex:2");
expect("primarySession is lowest live label", reg.primarySession(A)?.label === "codex:1");

// ── label allocation ─────────────────────────────────────────────────────────
const B = "AgentBBBBBBBBBBBBBBBBBBBBBBBBBBBB";
expect("fresh agent → codex:1", reg.allocateLabel(B) === "codex:1");
writeRaw(B, "codex:1", { pid: process.pid, updatedAt: now() });
expect("codex:1 live → allocate codex:2", reg.allocateLabel(B) === "codex:2");
writeRaw(B, "codex:2", { pid: process.pid, updatedAt: now() });
expect("codex:1&2 live → allocate codex:3", reg.allocateLabel(B) === "codex:3");
...
expect("requested colliding with live → next claude index", reg.allocateLabel(B, { requested: "codex:1" }) === "codex:3");

// reuse by harnessSessionId even when the prior entry is stale (restart case)
const C = "AgentCCCCCCCCCCCCCCCCCCCCCCCCCCCC";
writeRaw(C, "codex:5", { pid: deadPid, updatedAt: ago(120_000), harnessSessionId: "HSID-RESTART" });
expect("stale entry with same harnessSessionId → reuse its label", reg.allocateLabel(C, { harnessSessionId: "HSID-RESTART" }) === "codex:5");
expect("different harnessSessionId ignores stale label → codex:1", reg.allocateLabel(C, { harnessSessionId: "OTHER" }) === "codex:1");

// ── writePresence / removePresence round-trip (own pid, live) ─────────────────
const D = "AgentDDDDDDDDDDDDDDDDDDDDDDDDDDDD";
reg.writePresence(D, { label: "codex:1", harnessSessionId: "H1", cwd: "/x", startedAt: now() });
const dLive = reg.liveSessions(D);
expect("writePresence → one live session for us", dLive.length === 1 && dLive[0].label === "codex:1" && dLive[0].pid === process.pid);
reg.removePresence(D, "codex:1");
...
expect("concurrent claims are codex:1/2/3", [...uniqueLabels].sort().join(",") === "codex:1,codex:2,codex:3");
