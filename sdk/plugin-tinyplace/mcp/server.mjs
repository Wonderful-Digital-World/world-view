#!/usr/bin/env node
// tiny.place MCP server — ONE server, any harness.
//
// Wraps @tinyhumansai/tinyplace to give a harness session (Codex, Claude Code, …):
...
//
// The MCP process is long-lived for the whole harness session, so the listener
// and the active-wallet selection persist across turns. Every per-harness delta
// (data dir, session-id resolution, inbound push vs pull, instructions, session
// label prefix) is read from the active adapter — detected once at load via
// mcp/harness.mjs — so the 20 tools below are harness-agnostic.
...
// (data dir, session id, inbound push, instructions, label prefix) reads from it.
const ADAPTER = activeAdapter();
// Session-label prefix for the active harness (codex / claude); woven into tool
// descriptions so the examples the model sees match its own default labels.
const LP = ADAPTER.sessionLabelPrefix;
...
// Harness session identity — resolved per-harness by the adapter. Claude Code
// exposes CLAUDE_CODE_SESSION_ID to the MCP subprocess; Codex (verified codex-cli
// 0.142.5) does NOT pass any session-id env, so its adapter's resolver returns
// "" and we fall back to a per-process wrapper id generated once at boot. The
// Stop-hook dispatcher keys on the harness-provided session id on stdin; a
// mismatch with this process-local wrapper id is tolerated because the dispatcher
// falls back to `active-latest.json`.
const HARNESS_ENV_SESSION_ID = ADAPTER.resolveHarnessSessionId(); // real env id, or ""
...
const HARNESS_SESSION_ID =
  HARNESS_ENV_SESSION_ID ||
  `tp-${ADAPTER.provider}-${Date.now().toString(36)}-${randomBytes(4).toString("hex")}`;
...
// real harness session id when present, else the adapter's project dir (stable
// per project), else a global default.
function scopeKey() {
  if (HARNESS_ENV_SESSION_ID) return `session:${HARNESS_ENV_SESSION_ID}`;
  const proj = ADAPTER.projectDir();
...
async function adopt(walletName, { label } = {}) {
  ...
  // (use label:… / TINYPLACE_SESSION_LABEL), else claim the lowest free <harness>:n.
...
// ── MCP server ───────────────────────────────────────────────────────────────
// Capabilities: a harness that supports server→client push (Claude's channel)
// declares it here so inbound DMs can be pushed into a live session; a pull-only
// harness (Codex) declares none. Both keep the in-memory buffer + hook surfacing.
const MCP_OPTIONS = { instructions: ADAPTER.serverInstructions };
if (ADAPTER.inbound.push) {
  MCP_OPTIONS.capabilities = { experimental: { [ADAPTER.inbound.push.capability]: {} } };
}
const server = new McpServer({ name: "tinyplace", version: "0.1.0" }, MCP_OPTIONS);

// Real-time push is available only when the active harness advertises a channel
// (ADAPTER.inbound.push). On a pull-only harness this is false and inbound DMs
// still land in the in-memory buffer (served by the `inbox` tool) + the durable
// auto-responder queue; hooks surface them on the next turn.
const CHANNEL_PUSH = !!ADAPTER.inbound.push;

// Push an inbound DM into the active session as a channel event. No-op when the
// harness is pull-only (CHANNEL_PUSH=false) — buffer + hook injection carry it.
async function pushToChannel(msg) {
  if (!CHANNEL_PUSH) return;
  try {
    await server.server.notification({
      method: ADAPTER.inbound.push.method,
      params: {
        content:
...
server.registerTool(
  "use",
  {
    ...
    description: `Make a saved wallet the active agent for this session. Publishes its Signal key bundle + directory card (so peers can message it) and starts the background listener. Registers this session in the agent's session registry under a label (default ${LP}:<n>, or the one you pass). Pass remember:true to persist this wallet as the assignment for this session/scope so future runs auto-adopt it.`,
    inputSchema: {
      ...
      label: z.string().optional().describe(`Session label to register under (e.g. '${LP}:1'). Peers can target this session with to_session. Defaults to the lowest free ${LP}:<n>; a label already held by a live session falls back to the next index.`),
    },
  },
...
server.registerTool(
  "sessions",
  {
    ...
    description: `List the live sessions registered for the active agent, each with its label (e.g. ${LP}:1). A peer can target a specific one with the send tool's to_session. A session is live if it heartbeat within the liveness window and its process is alive.`,
    inputSchema: {},
  },
  async () => {
    try {
...
server.registerTool(
  "send",
  {
    ...
    inputSchema: {
      ...
      to_session: z.string().optional().describe(`Optional target session label of the peer (e.g. '${LP}:1'). Routes the message to that specific session of a multi-session peer.`),
...
server.registerTool(
  "auto_reply",
  {
    ...
    inputSchema: {
      ...
      to_session: z.string().optional().describe(`Optional target session label of the peer (e.g. '${LP}:1'). Reply back to the specific session the message came from.`),
      role: z.enum(["user", "agent"]).optional().describe("Authoring role for the envelope's message.role (default 'agent')."),
    },
  },
...
server.registerTool(
  "send_and_wait",
  {
    ...
    inputSchema: {
      to: z.string().describe("Recipient: @handle, base58 address, or base64 public key."),
      body: z.string().describe("Message text."),
      to_session: z.string().optional().describe(`Optional target session label of the peer (e.g. '${LP}:1').`),
      role: z.enum(["user", "agent"]).optional().describe("Authoring role for the envelope's message.role (default 'agent')."),
