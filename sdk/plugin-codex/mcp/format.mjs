...
// A session label is a short, token-like string (e.g. `codex:1`, `reviewer`).
...
// §15 default: a plugin session's harness_session_id is the Codex session id.
// Codex does not (yet, verified codex-cli 0.142.5) guarantee a session-id env to
// the MCP subprocess, so we try the plausible Codex vars and fall back to a
// caller-supplied override; the MCP server self-generates a wrapper id when empty.
...
export function sessionLabel() {
  return process.env.TINYPLACE_SESSION_LABEL?.trim() || "codex:1";
}
...
export function encodeEnvelope(opts) {
  ...
  const envelope = {
    ...
    harness: { provider: "codex", command: "tinyplace-codex-plugin", argv: [] },
    message: {
