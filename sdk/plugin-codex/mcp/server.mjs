...
// Codex session identity. Unlike Claude Code (CLAUDE_CODE_SESSION_ID), Codex is
// NOT confirmed (codex-cli 0.142.5) to expose a session id ENV to the MCP
// subprocess — so we try the plausible Codex vars, then fall back to a
// per-process wrapper id generated once at boot. Codex DOES pass session_id to
...
async function adopt(walletName, { label } = {}) {
  ...
  // (use label:… / TINYPLACE_SESSION_LABEL), else claim the lowest free codex:n.
...
const server = new McpServer(
  ...
  {
    // Codex is a PULL-ONLY MCP client (codex-cli 0.142.5): it does not honor
    // server-initiated notifications, so there is no real-time channel push.
    // Inbound DMs are surfaced two ways instead: the agent drains them with the
    // `inbox` tool, and the plugin's UserPromptSubmit/SessionStart hooks inject
    // newly-arrived DMs as additionalContext on the next turn.
    instructions:
...
// Codex has no server→client channel, so real-time push is disabled. Inbound
// DMs still land in the in-memory buffer (served by the `inbox` tool) and the
// durable auto-responder queue; hooks surface them on the next turn. Flip this
// if Codex ever gains notification support.
const CHANNEL_PUSH = false;

// Push an inbound DM into the active session as a channel event. No-op on Codex
// (CHANNEL_PUSH=false) — the buffer + hook injection carry inbound instead.
...
server.registerTool(
  "use",
  {
    ...
    description: "Make a saved wallet the active agent for this session. Publishes its Signal key bundle + directory card (so peers can message it) and starts the background listener. Registers this session in the agent's session registry under a label (default codex:<n>, or the one you pass). Pass remember:true to persist this wallet as the assignment for this session/scope so future runs auto-adopt it.",
    inputSchema: {
      ...
      label: z.string().optional().describe("Session label to register under (e.g. 'codex:1'). Peers can target this session with to_session. Defaults to the lowest free codex:<n>; a label already held by a live session falls back to the next index."),
    },
  },
...
server.registerTool(
  "sessions",
  {
    ...
    description: "List the live sessions registered for the active agent, each with its label (e.g. codex:1). A peer can target a specific one with the send tool's to_session. A session is live if it heartbeat within the liveness window and its process is alive.",
    inputSchema: {},
  },
  async () => {
    try {
...
server.registerTool(
  "send",
  {
    ...
    inputSchema: {
      ...
      to_session: z.string().optional().describe("Optional target session label of the peer (e.g. 'codex:1'). Routes the message to that specific session of a multi-session peer."),
...
server.registerTool(
  "auto_reply",
  {
    ...
    inputSchema: {
      ...
      to_session: z.string().optional().describe("Optional target session label of the peer (e.g. 'codex:1'). Reply back to the specific session the message came from."),
      role: z.enum(["user", "agent"]).optional().describe("Authoring role for the envelope's message.role (default 'agent')."),
    },
  },
...
server.registerTool(
  "send_and_wait",
  {
    ...
    inputSchema: {
      to: z.string().describe("Recipient: @handle, base58 address, or base64 public key."),
      body: z.string().describe("Message text."),
      to_session: z.string().optional().describe("Optional target session label of the peer (e.g. 'codex:1')."),
      role: z.enum(["user", "agent"]).optional().describe("Authoring role for the envelope's message.role (default 'agent')."),
