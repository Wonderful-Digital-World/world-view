...
// spawns one `codex exec` responder per message. Returns immediately so it never
// blocks the session.
//
...
// Test seam: claim the batch but skip spawning the responder (no `codex` needed).
if (process.env.TINYPLACE_DISPATCH_DRYRUN) {
  process.stdout.write(JSON.stringify({ claimed, batchDir }) + "\n");
  process.exit(0);
}
